...
public class SysMenuController extends BaseController {
	...
	public ModelAndView sysMenuEditIndex(HttpServletRequest request,
		...
		if (menu.getMenuParentId()>0){
			SysMenu menuParent = sysMenuService.findSysMenuById(menu.getMenuParentId().toString());
			map.put("menuParent", menuParent);
			map.put("menuParentValue", "1");
		}else{
			map.put("menuParentValue", "0");
		}
		Collection<SysMenu> menulist = sysMenuService.findAllSysMenu();
		map.put("menulist", menulist);
		map.put("menu", menu);
	...
	public JsonView sysMenuTreeData(HttpServletRequest request, String roleId)
		...
		try {
			...
			Collection<SysMenu> allMenulist = sysMenuService
					.findAllUsefulSysMenu();
